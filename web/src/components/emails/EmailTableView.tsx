...
export default function EmailTableView(props: EmailTableViewProps) {
  ...
  return (
    <div className="grid grid-flow-dense grid-cols-[min-content,1fr,1fr] md:grid-cols-[min-content,1fr,4fr,1fr] items-center select-none rounded bg-neutral-50 py-1 shadow-md dark:bg-neutral-800 dark:text-neutral-300 md:rounded-md">
      {emails.map((email) => {
        return (
          <EmailTableRow
            key={email.messageID}
